...
typedef NS_ENUM(NSInteger,JDPlayerState)
{
    ...
    JDPlayerStateLoading,           //加载中。。。
    JDPlayerStatePlaying,           //正在播放
    JDPlayerStatePaused,            //暂停
    JDPlayerStateError,             //错误
...
- (void)videoPlayer:(JDPlayer *)videoPlayer isBuffering:(BOOL)buffering;//正在缓冲中
...
- (void)videoPlayer:(JDPlayer*)videoPlayer didNextVideoButtonPressed:(JDVideoModel *)videoModel;/** 下一个*/
- (void)videoPlayer:(JDPlayer*)videoPlayer didPreviousVideoButtonPressed:(JDVideoModel *)videoModel;//上一个视频

- (void)videoPlayer:(JDPlayer *)videoPlayer didVideoQualityButtonPressed:(JDVideoModel *)videoModel;//视频质量更改
- (void)videoPlayer:(JDPlayer *)videoPlayer videoModel:(JDVideoModel *)videoModel reachabilityChanged:(NetworkStatus)status;

- (void)handleErrorCode:(JDPlayerErrorCode)errorCode videoModel:(JDVideoModel *)videoModel customMessage:(NSString*)customMessage;//播放错误的时候提示的自定义消息
