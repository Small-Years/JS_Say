...
extern NSString *const kReachabilityChangedNotifications;
...
- (void)addObservers
{
    ...
    [defaultCenter addObserver:self selector:@selector(reachabilityChanged:) name:kReachabilityChangedNotifications object:nil];
}
...
-(void)doubleTapClicked{//双击效果
//    [self didPlayButtonPressed];
    if (self.jdView.isFullScreen) {
        [self.jdView setIsFullScreen:NO];
        self.jdView.titleLabel.hidden = YES;
    }else{
        [self.jdView setIsFullScreen:YES];
        self.jdView.titleLabel.hidden = NO;
    }
}


- (void)didFullScreenButtonPressed{
    if ([self.delegate respondsToSelector:@selector(didFullScreenButtonClicked)]) {
        [self.delegate didFullScreenButtonClicked];
    }
}
...
//        [nav popViewControllerAnimated:YES];
        if ([self.delegate respondsToSelector:@selector(didDoneBtnClicked)]) {
            [self.delegate didDoneBtnClicked];
        }
    }
}

- (void)didSwipingHorizontal{//滑动快进和后退
//    NSLog(@"这里是什么？didSwipingHorizontal");
}
