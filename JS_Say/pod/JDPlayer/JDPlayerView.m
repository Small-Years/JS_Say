...
- (void)addChildConstraints
{
    ...
    [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(self.rewindButton.mas_left);
    }];
...
//    self.portraitFrame  = CGRectMake(0, 0, MIN(bounds.size.width, bounds.size.height), MAX(bounds.size.width, bounds.size.height));
    ...
    self.portraitFrame = CGRectMake(0, 0, SCREEN_WIDTH, VideoHeight);
...
-(void)handleDoubleTap{
    if ([self.delegate respondsToSelector:@selector(doubleTapClicked)]) {
        [self.delegate  doubleTapClicked];
    }
}
...
- (void)addGesture
{
    ...
    [self.playerLayerView addGestureRecognizer:doubleTap];
