...
//
    ...
    [self.navigationController.navigationBar setBackgroundImage:[UIImage imageNamed:@"默认导航栏"]  forBarMetrics:UIBarMetricsDefault];
}
...
//    这里需要创建一个tablview覆盖掉原来的东西除非是再次点击了搜索，并且搜索结果为空，才能把hidden设置为yes
    ...
    NSString *url = [NSString stringWithFormat:@"%@/select/camps?pageInvertedIndex=0&pageSize=10&repeatSubmit=62851721&signature=8EE76EC69261A384B08A04469D88601E&title=%@",kMainUrl,[AllMethod encodeString:searchBar.text]];
    NSLog(@"---%@",url);
...
-(void)loadMoreData{
    NSString *url = [NSString stringWithFormat:@"%@/select/camps?pageInvertedIndex=%d&pageSize=%d&repeatSubmit=62851721&signature=8EE76EC69261A384B08A04469D88601E&title=%@",kMainUrl,pageIndex,pageSize,[AllMethod encodeString:searchStr]];
    [NetWorkManager requestWithType:HttpRequestTypeGet withUrlString:url withParaments:nil withSuccessBlock:^(NSDictionary *responseObject) {
        if ([[responseObject objectForKey:@"desc"] isEqualToString:@"成功"]) {
            NSDictionary *dict = [responseObject objectForKey:@"data"];
            NSString *pageNum = [dict objectForKey:@"pageInvertedIndex"];
...
-(void)delaDataAndUpdateView{
    NSLog(@"%@",self.dataArray);
    if (self.dataArray.count == 0) {
        [AllMethod showAltMsg:@"暂时没有数据" WithController:self WithAction:nil];
        mainTableView.hidden = YES;
    }else{
...
//        NSLog(@"----%@",dict);
        ...
        vc.newsID = [dict objectForKey:@"id"];
