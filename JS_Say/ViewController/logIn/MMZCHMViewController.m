...
-(void)createTextFields
{
    ...
    [self.view addSubview:label];
        ...
        code=[self createTextFielfFrame:CGRectMake(100, 60, 90, 30) font:[UIFont systemFontOfSize:14]  placeholder:@"6位验证码" ];
...
-(void)zhuceBtnClicked{
    ...
    if ([code.text isEqualToString:@""]) {
        ...
        [WSProgressHUD showErrorWithStatus:@"验证码输入不正确"];
        return;
    }
    
    [AVUser signUpOrLoginWithMobilePhoneNumberInBackground:phone.text smsCode:code.text block:^(AVUser * _Nullable user, NSError * _Nullable error) {
        if (error) {
            NSLog(@"注册或者登陆：error:%@",error);
        }else{
            NSLog(@"注册或者登陆：user:%@",user);
        }
    }];
    
//    [AVUser signUpOrLoginWithMobilePhoneNumberInBackground:phone.text msCode:code.text block:^(AVUser *user, NSError *error) {
...
- (void)getValidCode:(UIButton *)sender
{
    ...
    [AVOSCloud requestSmsCodeWithPhoneNumber:phone.text callback:^(BOOL succeeded, NSError *error) {
        if (succeeded) {
