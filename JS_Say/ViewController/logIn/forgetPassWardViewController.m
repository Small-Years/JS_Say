...
-(void)createTextFields
{
    ...
    [yzButton addTarget:self action:@selector(getValiCode:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)getValiCode:(UIButton *)btn{
    if ([_phone.text isEqualToString:@""])
    {
        [WSProgressHUD showErrorWithStatus:@"亲,请输入手机号码"];
        return;
    }
    else if (_phone.text.length <11)
    {
        [WSProgressHUD showErrorWithStatus:@"您输入的手机号码格式不正确"];
        return;
    }
    ...
    [AVUser requestPasswordResetWithPhoneNumber:_phone.text block:^(BOOL succeeded, NSError *error) {
        if (succeeded) {
            ...
            [WSProgressHUD showErrorWithStatus:@""];
        }
    }];
   }
