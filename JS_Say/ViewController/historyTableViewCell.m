...
+ (instancetype)cellWithTableView:(UITableView *)tableView withIdentifier:(NSString *)identifier WithDict:(NSDictionary *)infoDict withRow:(NSInteger)row{
    ...
    if (cell == nil) {
        cell = [[historyTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier WithDict:infoDict withRow:row];
    }
    return cell;
}
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier WithDict:(NSDictionary *)infoDict withRow:(NSInteger)row{
    ...
    if (self){
        ...
        NSString *iconUrl = [infoDict objectForKey:@"picOne"];
...
//        timeLable.text = [AllMethod changeDateMethod:[infoDict objectForKey:@"timeAgo"] From:@"yyyy-MM-dd HH:mm:ss" To:@"MM-dd HH:mm"];
        timeLable.text = [infoDict objectForKey:@"timeAgo"];
